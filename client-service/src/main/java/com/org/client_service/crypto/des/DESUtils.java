package com.org.client_service.crypto.des;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.Collections;
import java.util.List;

/**
 * Для дешифровки данных
 */
public class DESUtils {
    // Таблицы перестановок и S-боксы (полные)
    private static final int[] PC1 = {
            57, 49, 41, 33, 25, 17, 9,
            1, 58, 50, 42, 34, 26, 18,
            10, 2, 59, 51, 43, 35, 27,
            19, 11, 3, 60, 52, 44, 36,

            63, 55, 47, 39, 31, 23, 15,
            7, 62, 54, 46, 38, 30, 22,
            14, 6, 61, 53, 45, 37, 29,
            21, 13, 5, 28, 20, 12, 4
    };

    private static final int[] PC2 = {
            14, 17, 11, 24, 1, 5,
            3, 28, 15, 6, 21, 10,
            23, 19, 12, 4, 26, 8,
            16, 7, 27, 20, 13, 2,

            41, 52, 31, 37, 47, 55,
            30, 40, 51, 45, 33, 48,
            44, 49, 39, 56, 34, 53,
            46, 42, 50, 36, 29, 32
    };

    private static final int[] E_BOX = {
            32, 1, 2, 3, 4, 5,
            4, 5, 6, 7, 8, 9,
            8, 9, 10, 11, 12, 13,
            12, 13, 14, 15, 16, 17,
            16, 17, 18, 19, 20, 21,
            20, 21, 22, 23, 24, 25,
            24, 25, 26, 27, 28, 29,
            28, 29, 30, 31, 32, 1
    };

    private static final int[] P_BOX = {
            16, 7, 20, 21,
            29, 12, 28, 17,
            1, 15, 23, 26,
            5, 18, 31, 10,
            2, 8, 24, 14,
            32, 27, 3, 9,
            19, 13, 30, 6,
            22, 11, 4, 25
    };

    private static final int[] IP = {
            58, 50, 42, 34, 26, 18, 10, 2,
            60, 52, 44, 36, 28, 20, 12, 4,
            62, 54, 46, 38, 30, 22, 14, 6,
            64, 56, 48, 40, 32, 24, 16, 8,
            57, 49, 41, 33, 25, 17, 9, 1,
            59, 51, 43, 35, 27, 19, 11, 3,
            61, 53, 45, 37, 29, 21, 13, 5,
            63, 55, 47, 39, 31, 23, 15, 7
    };

    private static final int[] FP = {
            40, 8, 48, 16, 56, 24, 64, 32,
            39, 7, 47, 15, 55, 23, 63, 31,
            38, 6, 46, 14, 54, 22, 62, 30,
            37, 5, 45, 13, 53, 21, 61, 29,
            36, 4, 44, 12, 52, 20, 60, 28,
            35, 3, 43, 11, 51, 19, 59, 27,
            34, 2, 42, 10, 50, 18, 58, 26,
            33, 1, 41, 9, 49, 17, 57, 25
    };

    private static final int[] ROTATIONS = {
            1, 1, 2, 2, 2, 2, 2, 2,
            1, 2, 2, 2, 2, 2, 2, 1
    };

    private static final int[][][] S_BOXES = {
            {
                    {14,4,13,1,2,15,11,8,3,10,6,12,5,9,0,7},
                    {0,15,7,4,14,2,13,1,10,6,12,11,9,5,3,8},
                    {4,1,14,8,13,6,2,11,15,12,9,7,3,10,5,0},
                    {15,12,8,2,4,9,1,7,5,11,3,14,10,0,6,13}
            },
            {
                    {15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10},
                    {3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5},
                    {0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15},
                    {13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9}
            },
            {
                    {10,0,9,14,6,3,15,5,1,13,12,7,11,4,2,8},
                    {13,7,0,9,3,4,6,10,2,8,5,14,12,11,15,1},
                    {13,6,4,9,8,15,3,0,11,1,2,12,5,10,14,7},
                    {1,10,13,0,6,9,8,7,4,15,14,3,11,5,2,12}
            },
            {
                    {7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15},
                    {13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9},
                    {10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4},
                    {3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14}
            },
            {
                    {2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9},
                    {14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6},
                    {4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14},
                    {11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3}
            },
            {
                    {12,1,10,15,9,2,6,8,0,13,3,4,14,7,5,11},
                    {10,15,4,2,7,12,9,5,6,1,13,14,0,11,3,8},
                    {9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6},
                    {4,3,2,12,9,5,15,10,11,14,1,7,6,0,8,13}
            },
            {
                    {4,11,2,14,15,0,8,13,3,12,9,7,5,10,6,1},
                    {13,0,11,7,4,9,1,10,14,3,5,12,2,15,8,6},
                    {1,4,11,13,12,3,7,14,10,15,6,8,0,5,9,2},
                    {6,11,13,8,1,4,10,7,9,5,0,15,14,2,3,12}
            },
            {
                    {13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7},
                    {1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2},
                    {7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8},
                    {2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11}
            }
    };


    public static byte[] pkcs5Pad(byte[] data, int blockSize) {
        int paddingLen = blockSize - (data.length % blockSize);
        byte[] padded = Arrays.copyOf(data, data.length + paddingLen);
        Arrays.fill(padded, data.length, padded.length, (byte) paddingLen);
        return padded;
    }

    public static byte[] pkcs5Unpad(byte[] data) {
        int pad = data[data.length - 1] & 0xFF;
        return Arrays.copyOf(data, data.length - pad);
    }

    public static List<Integer> bytesToBits(byte[] bytes) {
        List<Integer> bits = new ArrayList<>();
        for (byte b : bytes)
            for (int i = 7; i >= 0; i--)
                bits.add((b >> i) & 1);
        return bits;
    }

    public static byte[] bitsToBytes(List<Integer> bits) {
        byte[] bytes = new byte[bits.size() / 8];
        for (int i = 0; i < bits.size(); i++) {
            int byteIndex = i / 8;
            bytes[byteIndex] <<= 1;
            bytes[byteIndex] |= bits.get(i);
        }
        return bytes;
    }

    public static List<Integer> permute(List<Integer> bits, int[] table) {
        List<Integer> output = new ArrayList<>();
        for (int value : table) {
            output.add(bits.get(value - 1));
        }
        return output;
    }

    public static List<Integer> leftRotate(List<Integer> bits, int n) {
        List<Integer> rotated = new ArrayList<>(bits);
        Collections.rotate(rotated, -n);
        return rotated;
    }

    public static List<List<Integer>> generateRoundKeys(byte[] key) {
        List<Integer> keyBits = bytesToBits(key);
        List<Integer> permKey = permute(keyBits, PC1);
        List<Integer> C = new ArrayList<>(permKey.subList(0, 28));
        List<Integer> D = new ArrayList<>(permKey.subList(28, 56));
        List<List<Integer>> roundKeys = new ArrayList<>();

        for (int i = 0; i < 16; i++) {
            C = leftRotate(C, ROTATIONS[i]);
            D = leftRotate(D, ROTATIONS[i]);
            List<Integer> combined = new ArrayList<>(C);
            combined.addAll(D);
            roundKeys.add(permute(combined, PC2));
        }
        return roundKeys;
    }

    public static List<Integer> feistel(List<Integer> R, List<Integer> roundKey) {
        List<Integer> expanded = permute(R, E_BOX);
        for (int i = 0; i < expanded.size(); i++) {
            expanded.set(i, expanded.get(i) ^ roundKey.get(i));
        }

        List<Integer> output = new ArrayList<>();
        for (int i = 0; i < 8; i++) {
            List<Integer> block = expanded.subList(i * 6, i * 6 + 6);
            int row = (block.get(0) << 1) | block.get(5);
            int col = (block.get(1) << 3) | (block.get(2) << 2) | (block.get(3) << 1) | block.get(4);
            int val = S_BOXES[i][row][col];
            for (int j = 3; j >= 0; j--) {
                output.add((val >> j) & 1);
            }
        }

        return permute(output, P_BOX);
    }

    public static List<Integer> desBlockEncrypt(List<Integer> bits, List<List<Integer>> roundKeys) {
        List<Integer> block = permute(bits, IP);
        List<Integer> L = new ArrayList<>(block.subList(0, 32));
        List<Integer> R = new ArrayList<>(block.subList(32, 64));

        for (int i = 0; i < 16; i++) {
            List<Integer> temp = R;
            R = new ArrayList<>();
            List<Integer> f = feistel(temp, roundKeys.get(i));
            for (int j = 0; j < 32; j++) {
                R.add(L.get(j) ^ f.get(j));
            }
            L = temp;
        }

        List<Integer> combined = new ArrayList<>(R);
        combined.addAll(L);
        return permute(combined, FP);
    }

    public static List<Integer> desBlockDecrypt(List<Integer> bits, List<List<Integer>> roundKeys) {
        List<Integer> block = permute(bits, IP);
        List<Integer> L = new ArrayList<>(block.subList(0, 32));
        List<Integer> R = new ArrayList<>(block.subList(32, 64));

        for (int i = 15; i >= 0; i--) {
            List<Integer> temp = R;
            R = new ArrayList<>();
            List<Integer> f = feistel(temp, roundKeys.get(i));
            for (int j = 0; j < 32; j++) {
                R.add(L.get(j) ^ f.get(j));
            }
            L = temp;
        }

        List<Integer> combined = new ArrayList<>(R);
        combined.addAll(L);
        return permute(combined, FP);
    }

    public static byte[] desEncrypt(byte[] key, byte[] plaintext) {
        List<List<Integer>> roundKeys = generateRoundKeys(key);
        byte[] padded = pkcs5Pad(plaintext, 8);
        byte[] result = new byte[padded.length];

        for (int i = 0; i < padded.length; i += 8) {
            byte[] block = Arrays.copyOfRange(padded, i, i + 8);
            List<Integer> bits = bytesToBits(block);
            List<Integer> encryptedBits = desBlockEncrypt(bits, roundKeys);
            byte[] encryptedBytes = bitsToBytes(encryptedBits);
            System.arraycopy(encryptedBytes, 0, result, i, 8);
        }

        return result;
    }

    public static byte[] desDecrypt(byte[] key, byte[] ciphertext) {
        List<List<Integer>> roundKeys = generateRoundKeys(key);
        byte[] result = new byte[ciphertext.length];

        for (int i = 0; i < ciphertext.length; i += 8) {
            byte[] block = Arrays.copyOfRange(ciphertext, i, i + 8);
            List<Integer> bits = bytesToBits(block);
            List<Integer> decryptedBits = desBlockDecrypt(bits, roundKeys);
            byte[] decryptedBytes = bitsToBytes(decryptedBits);
            System.arraycopy(decryptedBytes, 0, result, i, 8);
        }

        return pkcs5Unpad(result);
    }
}
